...
class ASTParser:
    ...
    def _get_enclosing_function(self, node):
        ...
        for ancestor in ast.walk(self.ast_tree):
            if (isinstance(ancestor, ast.FunctionDef) and 
                node.lineno >= ancestor.lineno and
                (not ancestor.body or node.lineno <= ancestor.body[-1].lineno)):
    ...
    def _is_zero_in_scope(self, var_name: str, node: ast.AST) -> bool:
        ...
        if var_name in self.var_assignments:
            ...
            logger.debug(f"Found assignment: {val} in scope {self.var_assignments[var_name]['scope']}")
            if val == 0:
                return True
            ...
            for call in self.function_calls[func]:
                if 'param_map' in call and var_name in call['param_map']:
                    ...
                    if isinstance(param_value, str):
                        ...
                        if (param_value in self.var_assignments and
                            self.var_assignments[param_value]['value'] == 0):
                            return True
